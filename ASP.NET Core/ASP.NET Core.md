...
- 异步方法在执行时 遇到 await 后，当前线程不阻塞，会默认切换开辟一个工作线程 在这里阻塞等待 ，当前线程回退到主线程继续执行
- 异步的实现原理见书  28
...
**Updata-database** 	:  编译并执行 数据库的迁移代码
...
当系统出现未经处理的异常时，异常过滤器就会执行，就可以在异常过滤器中   对异常进行处理
